...
from angelia.handlers.helpers.images import query_all_images
...
from angelia.lib.images import ImageTypes
from angelia.lib.loyalty_card import LoyaltyCardStatus, StatusName
from angelia.lib.payment_card import PllLinkState, WalletPLLSlug
from angelia.lib.vouchers import MAX_INACTIVE, VoucherState, voucher_state_names
...
class VoucherDisplay:
    ...
    def __init__(self, raw_voucher: dict[str, dict[str, str]]) -> None:
        self.barcode_type = raw_voucher.get("barcode_type")
...
class WalletHandler(BaseHandler):
    ...
    def _scheme_account_query(self) -> "Select":
        return (
            select(
                ...
                SchemeAccountUserAssociation.authorised,
                Scheme.barcode_type,
                Scheme.colour,
                Scheme.text_colour,
    ...
    def get_loyalty_card_transactions_response(self, loyalty_card_id: int) -> dict:
        ...
        match LoyaltyCardStatus.get_status_dict(query_dict["link_status"]).get("api2_state"), query_dict["authorised"]:
            case (StatusName.AUTHORISED, _) | (StatusName.DEPENDANT, True):
